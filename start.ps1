...
function Ensure-Command($Command, $WingetId, $DisplayName) {
    ...
    if (-not (Get-Command $Command -ErrorAction SilentlyContinue)) {
        throw "$DisplayName installation failed. Check the winget output and retry."
    }
}
...
function Ensure-NodeVersion {
    ...
    if ($major -lt 20) {
        throw "Node.js $nodeVersion is too old. Version 20+ is required."
    }
}
...
function Ensure-SystemDeps {
    ...
    try {
        ...
        if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
            throw "winget was not found, so Python 3.12 cannot be installed automatically."
        }
        Write-Log "Installing Python 3.12"
        winget install --accept-package-agreements --accept-source-agreements -e --id "Python.Python.3.12"
        Refresh-Path
    }
    ...
    if (-not (Test-Path $ProviderServerDir)) {
        throw "Provider directory is missing: $ProviderServerDir"
    }
    [void](Get-PythonLauncher)
}
...
function Ensure-Venv {
    ...
    if (-not (Test-Path $venvPython)) {
        ...
        Write-Log "Creating Python virtual environment with $($python.Command) $($python.Arguments -join ' ')"
...
function Install-ProviderServer {
    Write-Log "Installing and building the bgutil provider server"
...
function Start-App {
    Write-Log "Starting app at http://$HostAddress`:$Port"
